  ...
  getAuth:function(e){
    ...
    if (e.detail.userInfo) {
      ...
      app.isLogin().then(function(res){
          ...
          let info = common.getLogin(app.globalData.config['reqlogin'],nickName, avatarUrl);
          console.log(self.data.userInfo)
          info.then((res) => {
            self.setData({
              isOnTel:res.phone?true:false
            })
            let obj = {
              member_id:res.id?res.id:''
            }
              //获取课程列表
            common.reqUrl(app.globalData.config['reqOrder'], 'POST', obj, self.orderSuccess, self.doFail)
            //获取优惠券
            common.reqUrl(app.globalData.config['reqCardList'], 'POST', obj, self.listSuccess, self.doFail)
          })
        },function(err){
          console.log(err)
        })
